...
# distributed on an "License for the specific
...
def enable_gpu(enable: bool = True) -> bool:
    ...
    Args:
        enable: Whether to enable GPU usage
...
def _apply_controlled_phase_shift_gpu_inplace(
    state_gpu, phase_factor: complex, controls, target: int
) -> tuple[None, bool]:
...
@nb.njit(parallel=True, fastmath=True, cache=True, nogil=True)
def _apply_diagonal_gate_large(
    state: np.ndarray, matrix: np.ndarray, target: int, out: np.ndarray
) -> tuple[np.ndarray, bool]:
    """
...
@nb.njit(parallel=True, fastmath=True, cache=True, nogil=True)
def _apply_hadamard_gate_large(
    state: np.ndarray, matrix: np.ndarray, target: int, out: np.ndarray
) -> tuple[np.ndarray, bool]:
...
def _apply_cnot_gpu(
    ...
    temp = state_gpu[slices_c1t0].copy()
    state_gpu[slices_c1t0] = state_gpu[slices_c1t1]
    state_gpu[slices_c1t1] = temp

    if isinstance(state, np.ndarray):
        state[:] = _to_cpu(state_gpu)
        return state, False
    else:
        return _to_cpu(state_gpu), False
...
def _apply_single_qubit_gate_gpu_inplace(
    state_gpu, out_gpu, matrix: np.ndarray, target: int, gate_type: str = None
) -> tuple[None, bool]:
    """GPU-accelerated single qubit gate implementation for device arrays."""
    n_qubits = len(state_gpu.shape)
    
    if gate_type and gate_type in DIAGONAL_GATES:
        return _apply_diagonal_gate_gpu_inplace(state_gpu, matrix, target, out_gpu)
    else:
        a, b, c, d = matrix[0, 0], matrix[0, 1], matrix[1, 0], matrix[1, 1]
        n = n_qubits - target - 1
        mask = (1 << n) - 1
