...
def validate_time_separation(times: List[Decimal], min_time_separation: Decimal, name: str):
    for i in range(len(times) - 1):
        ...
        if time_diff < min_time_separation:
            raise ValueError(
                f"Time points of {name} time_series, {i} ({times[i]}) and {i+1} ({times[i+1]}), are too close; they are separated by {time_diff} seconds. It must be at least {min_time_separation} seconds"
            )
...
def validate_max_absolute_slope(
    ...
    # Raise ValueError if at any time the time series (times, values) rises/falls faster than allowed
    for idx in range(len(values) - 1):
        slope = (values[idx + 1] - values[idx]) / (times[idx + 1] - times[idx])
        if abs(slope) > max_slope:
            raise ValueError(
                f"For the {name} field, rate of change of values (between the {idx}-th and the {idx+1}-th times) is {abs(slope)}, more than {max_slope}"
            )
