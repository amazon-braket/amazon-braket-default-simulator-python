...
def test_simulator_run_bell_pair(bell_ir, batch_size):
    simulator = JaqcdStateVectorSimulator()
    shots_count = 10000
    ...
    assert result.taskMetadata == TaskMetadata(
        id=result.taskMetadata.id, deviceId=JaqcdStateVectorSimulator.DEVICE_ID, shots=shots_count
    )
...
def test_simulator_bell_pair_result_types(bell_ir_with_result, targets, batch_size):
    ...
    assert result.taskMetadata == TaskMetadata(
        id=result.taskMetadata.id, deviceId=JaqcdStateVectorSimulator.DEVICE_ID, shots=0
    )
...
def test_observable_hash_tensor_product():
    ...
    hash_dict = JaqcdStateVectorSimulator._observable_hash(obs)
