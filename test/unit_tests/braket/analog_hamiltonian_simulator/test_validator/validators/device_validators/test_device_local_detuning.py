...
from braket.analog_hamiltonian_simulator.rydberg.validators.device_validators.\
    device_local_detuning import DeviceLocalDetuningValidator
...
def test_validation_no_detuning_data(mock_local_detuning_data, non_local_capabilities_constants):
    ...
    with pytest.raises(ValueError) as e:
        DeviceLocalDetuningValidator(capabilities=non_local_capabilities_constants, **mock_local_detuning_data)
...
@pytest.mark.parametrize(
    ...
    [
        (
            [0.0, 2.5e7, 0.0],
            [0.0, 0.01e-6, 2.0e-6],
            "For the magnitude field, rate of change of values (between the 0-th and the 1-th times) is 2500000000000000.0, more than 1256600000000000.0",
        ),
        (
            [0.0, 2.5e7, 2.5e7, 0.0],
...
@pytest.mark.parametrize(
    ...
    [
        (
            [0.0, 12.1e-9],
            "time point 1 (1.21E-8) of magnitude time_series is defined with too many digits; it must be an integer multiple of 1E-9",
        ),
        (
            [0.0, 12.1e-9, 4e-6],
            "time point 1 (1.21E-8) of magnitude time_series is defined with too many digits; it must be an integer multiple of 1E-9",
        ...
        (
            [0.0, 22.1e-9, 12.1e-9],
            "time point 1 (2.21E-8) of magnitude time_series is defined with too many digits; it must be an integer multiple of 1E-9",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            [0.0, 1.0, 2.0], 
            "The values of the shifting field magnitude time series at the first and last time points are 0.0, 2.0; they both must be both 0."
        ), 
        (
            [0.2, 1.0, 0], 
            "The values of the shifting field magnitude time series at the first and last time points are 0.2, 0; they both must be both 0."
        ), 
        (
            [0.0, 0.0, 1e-5], 
            "The values of the shifting field magnitude time series at the first and last time points are 0.0, 1e-05; they both must be both 0."
        ), 
    ]
)
def test_shifting_start_and_end_are_zero(times, error_message, mock_local_detuning_data, capabilities_with_local_rydberg):
