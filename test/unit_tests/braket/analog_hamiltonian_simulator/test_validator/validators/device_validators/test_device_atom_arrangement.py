...
from braket.analog_hamiltonian_simulator.rydberg.validators.device_validators.\
    device_atom_arrangement import DeviceAtomArrangementValidator

from braket.analog_hamiltonian_simulator.rydberg.validators.atom_arrangement \
    import AtomArrangement
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [1, 0],
            "Coordinates 1([2.2e-06, 7.31e-06]) is defined with too high precision; they must be multiples of 1E-7 meters",
        ),
        (
            [[3.201e-6, 0.0], [0.00, 4.1e-6], [-1.101e-6, 8.11e-6]],
            [1, 0, 1],
            "Coordinates 0([3.201e-06, 0.0]) is defined with too high precision; they must be multiples of 1E-7 meters",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [0, 1],
            "Sites [Decimal('0.0'), Decimal('0.0')] and site [Decimal('0.000005'), Decimal('0.0000024')] have y-separation (0.0000024). It must either be exactly zero or not smaller than 0.000004 meters",
        ),
        (
            [[Decimal("0.0"), Decimal("0.0")], [Decimal("5e-6"), Decimal("1.4e-6")]],
            [0, 1],
            "Sites [Decimal('0.0'), Decimal('0.0')] and site [Decimal('0.000005'), Decimal('0.0000014')] have y-separation (0.0000014). It must either be exactly zero or not smaller than 0.000004 meters",
        ),
    ],
)
# Rule: All sites in the lattice must be separated vertically by at least lattice.geometry.spacing_vertical_min
...
def test_atom_arrangement_filling_atom_number_limit(
    ...
    mock_atom_arrangement_data["filling"] = filling
    mock_atom_arrangement_data["sites"] = [[0, 0], [0, 0.00004], [0, 0.00005], [0.00005, 0.00004], [0.00005, 0]]
